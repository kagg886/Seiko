...
- **增强访问**的形式是`{集合变量.集合变量.数组(下标).变量}`。此形式允许你填写链式表达式来对尖括号内容求值，并替换成别的字符。例如：*(其中涉及的函数可以通过下面的函数表查看)*
...
| 变量检验 | 2        | `$变量检验 要存入的变量 要检验的变量 是否输出类名(可选)$` | 非阻断函数 | 检验变量是否为空<br />若选择输出类名，则结果是Java对象的类名 | `$变量检验 A B$`<br />`$变量检验 A B true$` |

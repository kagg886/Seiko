...
- **增强访问**的形式是`{根变量名.集合变量名.数组(下标).变量名}`。此形式允许你填写链式表达式来对尖括号内容求值，并替换成别的字符。例如：*(其中涉及的函数可以通过下面的函数表查看)*
...
自0.1.1起，Seiko支持在方括号表达式中嵌入基本初等函数。格式为:`function(值1,值2...)`，可用的function如下:
...
*除`数组创建`外，此表内其余方法均已过时，出于兼容性考虑保留此函数。*

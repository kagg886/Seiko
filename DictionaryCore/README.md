...
| 集合长  | 2    | `$集合长 存入变量 集合名$`        | 非阻断函数 | 获取一个集合的长度                            | `$集合长 A B$`                                                            |
...
| 数组导入 | 2    | `$数组导入 存入变量 数组字符串(json格式)$` | 非阻断函数 | 将一个字符串转成数组(写死的字符串支持空格)                   | `$数组导入 A [1,2,3,4,5]$`           |
| 数组删除 | 2    | `$数组删除 数组名 序号$`             | 非阻断函数 | 删除数组对应序号的内容                              | `$数组删除 A 0$`                     |
| 数组赋值 | 3或2  | `$数组赋值 数组名 添加的变量 序号(可选)$`   | 非阻断函数 | 若序号存在，则将对应序号的内容替换为新内容。<br />若序号不存在则在末尾追加 | `$数组赋值 A A$`<br />`$数组赋值 A A 2$` |
| 取数组  | 3    | `$取数组 数组名 序号 存入变量$`         | 非阻断函数 | 获取数组对应序号的内容。<br />类型可能为字符串，数字，集合或数组      | `$取数组 A 1 B$`                    |
| 数组长  | 2    | `$数组长 数组名 存入变量$`            | 非阻断函数 | 获取数组的长度                                  | `$数组长 A B$`                      |
| 数组转  | 2    | `$数组转 数组名 存入变量$`            | 非阻断函数 | 将数组转成json字符串                             | `$数组转 A B$`                      |
...
| 调用  | 1    | `$调用 方法名$` | 非阻断函数 | 调用词库内定义的词条       | 详见下方        |
...
> 4. 以下是示例：在这个示例中，会发送两条消息。第一条为`准备调用...`，第二条为`赋值完成`和`返回值:1`
...
| 访问         | 5或4或3  | `$访问 结果存入变量 请求方式 网址 头集合(可以使用null占位) 参数集合/参数体$` | 阻断函数   | 访问一个网址，并将结果转成字符串存入 头集合和参数集合为可选选项，传入类型为集合 | `访问 GET http://www.baidu.com`                              |
...
| 回复者   | 数字   | 若回复了别人，则变量的值是被回复的QQ |                      若没有回复则为空值                      |
...
- **时间类**(与时间转换有关的函数)
	...
	- 时间格式化
		- 调用方式:`$时间格式化 存入变量 毫秒时间戳$` 或 `$时间格式化 存入变量 不能是数字的占位符 年 月 日 时 分 秒 毫秒$`
		- 返回:时间字符串和时间对象
		- 注意: 
...
- **动态词库类**(可以在伪代码内动态调用别的伪代码的函数)
	- 装载
		- 调用方式: `$装载 伪代码文件路径 模块名(可选)$`
		- 注意事项:
			1. 伪代码文件路径相对于`dicData`目录下
			2. 子词库仍需要遵守SeikoDIC的规范，因此不要忘了添加:`#Seiko词库`
			3. 一次注册子词库在全部函数中均可被访问到，但子词库的全局变量独立于父词库
			4. 装载时会调用子词库的生命周期方法
			5. 当不填写模块名时，模块名为文件名，例如伪代码文件路径为`code/abc/seiko1.txt`时，模块名就为`seiko1.txt`
	- 卸载
		- 调用方式:`$卸载 模块名$`
		- 注意事项:
			1. 卸载时会调用子词库的生命周期方法
			2. 尽量避免在子词库内使用`异步调用`，这可能会导致注册的异步调用任务无法正常终止
	- 子文件调用
		- 调用方式:`$子文件调用 模块名 触发词 继承事件(真/假)(可选)$`
		- 注意事项:
			1. 若`继承事件`选项为真，则SeikoDIC会匹配子词库里与被调函数相同事件的函数
			
				这意味着开启此选项你可以调用子词库中事件类型非`函数`的方法
			
			2. 触发词可以为任意值，只要能匹配到子词库的正则表达式即可执行。
	
- **未分类的其他方法**

| 名字         | 参数个数 | 调用说明                                                     | 函数类型   | 说明                                                         | 调用示例                                                     |
| ------------ | -------- | ------------------------------------------------------------ | ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 随机数       | 3或2或1  | `$随机数 存入变量名 最小值(可选) 最大值(可选)$`<br />或<br />`$随机数 存入变量名 $` | 非阻断函数 | 生成一个随机数并存放到变量名中(左闭右开区间)<br />不填最大值按下界为0上界为所填值生成数据<br />什么都不填则生成0-1之间的浮点数。此时可以使用格式化字符串进行对小数点的取舍(看例子) | `$随机数 A 10 20$`<br />`$随机数 B 10$`<br/>`$随机数 C$`<br/>`$随机数 D %.2f$` |
...
| 设置捕错函数 | 1        | `$设置捕错函数 函数名$`                                      | 非阻断函数 | 设置一个函数，当伪代码引擎执行伪代码时遇到错误时调用。       | 见下方                                                       |
...
> ```
>
> 当你在后续的代码中遇到错误(**如BOT不是管理员权限等**，将会调用CATCH函数。您可以在CATCH里处理异常情况并返给用户对应的信息)
>
